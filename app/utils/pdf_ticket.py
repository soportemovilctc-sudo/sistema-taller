...
from app.utils.richtext import condiciones_a_texto_plano
...
def _condiciones_servicio(t, texto_condiciones=""):
    ...
    La tirilla siempre imprime este bloque en texto plano (sin negrita,
    cursiva, color ni alineación especial): si la configuración se guardó
    con el editor enriquecido, primero se convierte a texto plano."""
    condiciones = parse_condiciones_servicio(condiciones_a_texto_plano(texto_condiciones))
    if not condiciones:
        return
    t.separador()
