...
def calcular_total(cotizacion, recargo) -> Decimal:
    cotizacion = to_decimal(cotizacion)
    recargo = to_decimal(recargo)
    total = cotizacion + recargo
    if total < 0:
...
def recalcular_orden(orden):
    """Recalcula recargo, total y saldo de una orden (objeto SQLAlchemy) en memoria.
    No hace commit; quien llama decide cuándo guardar.
    """
    recargo = calcular_recargo(orden.cotizacion, orden.recargo_pct)
    total = calcular_total(orden.cotizacion, recargo)
